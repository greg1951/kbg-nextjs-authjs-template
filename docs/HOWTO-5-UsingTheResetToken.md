...
The functionality here provides a way to receive the password token via a request URL parameter and then to confirm the token. It does not include the logic to send the email. Here it assumes the request URL is in the email sent to the user and the user followed it to this page for the reset using the token.

The functionality here will follow these steps. The app pages are located in the `@/app/update-password` directories.
...
The `@/app/(logged-out)/update-password/page.tsx` file implements the page that will run from the email sent (not documented yet) that contains a password reset token. 
...
The page.tsx file in its entirety is provided below with notes after to describe key steps.
...
The `@/db/queries-passwordResetTokens.ts` file implements the server function shown below.
...
**Notes**:
  ...
  - **Note 2**: Query the passwordResetTokens table by the input token.

  - **Note 3**: Return email address and boolean containing whether it is still valid.

# Step 3: Create Form to Reset Password
The form is implemented in the `@/app/(logged-out)/update-password/update-password-form/index.tsx` file.
...
The `@/app/(logged-out)/update-password/update-password-form/action.ts` file implements the `updateResetPassword` server function called from the `handleSubmit` in the `index.tsx` file.
...
**Notes**:
  ...
  - **Note 1**: As a precaution, check if a session already exists, if so, then set an error return that will be surfaced by the `index.tsx` --> `page.tsx` implementations.
